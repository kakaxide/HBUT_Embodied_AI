...
# Ensure CUDA libraries are findable by subprocesses
export LD_LIBRARY_PATH="/usr/local/corex/lib64:/usr/local/iluvatar/lib64:${LD_LIBRARY_PATH:-}"

setup_sim_env() {
    export MUJOCO_GL="${MUJOCO_GL:-egl}"
    export PYOPENGL_PLATFORM="${PYOPENGL_PLATFORM:-egl}"
    # Only a single (software) EGL device is exposed on CoreX/Iluvatar, so every
    # env process must render through device 0. GPU ids are still assigned to
    # rollout workers via CUDA_VISIBLE_DEVICES for model inference.
    export MUJOCO_EGL_DEVICE_ID="${MUJOCO_EGL_DEVICE_ID:-0}"
    export RLINF_SOFTWARE_EGL="${RLINF_SOFTWARE_EGL:-1}"
