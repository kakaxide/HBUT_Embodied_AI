...
SUPPORTED_PLATFORMS=("nvidia" "amd" "ascend" "tianshu")
...
# 天数 (Tianshu) GPU mode: skip venv creation, preserve +corex packages
TIANSHU_MODE=0
...
Common options:
    ...
    --tianshu              Tianshu (天数) GPU mode: install to system Python directly,
                           preserve existing +corex packages (torch, vllm, flash-attn, etc.).
...
configure_tianshu() {
    # Tianshu (天数) GPU: torch/vllm/flash-attn/apex/xformers/triton/ray
    # and many other packages are provided as +corex builds pre-installed
    # in the system Python. We must NOT overwrite any of them with CUDA
    # or CPU builds from PyPI. This platform installs directly into the
    # system environment (no venv) and skips all torch-family overrides.
    PLATFORM_TORCH_STR=""
    PLATFORM_TORCH_INDEX=""
    PLATFORM_TORCH_PACKAGES=()
    PLATFORM_VENV_EXPORTS=()
    # 天数 provides its own flash-attn (+corex) and apex (+corex)
    PLATFORM_FLASH_ATTN_INSTALL=0
    PLATFORM_FLASH_ATTN_PREBUILT=0
    PLATFORM_RELAX_TORCHCODEC=1
    PLATFORM_EXTRA_OVERRIDES=()
    # Unset UV_TORCH_BACKEND entirely — let pip pick from system packages,
    # not force a CUDA or CPU torch download.
    unset UV_TORCH_BACKEND
    # System install: tell uv to use system Python instead of requiring a venv.
...
install_tianshu_extras() {
    # 天数 provides its own +corex builds of torch, vllm, flash-attn,
    # apex, triton, xformers, ray, etc. Nothing to install here — all
    # platform-specific packages are already in the system Python.
    echo "[install.sh] Tianshu (天数) platform: relying on pre-installed +corex packages."
}
...
install_platform_extras() {
    case "$PLATFORM" in
        ...
        tianshu)  install_tianshu_extras ;;
    esac
}
...
apply_sglang_override() {
    if [ "$TIANSHU_MODE" -eq 1 ]; then
        echo "[install.sh] Tianshu mode: skipping sglang override (preserving +corex builds)."
        return 0
    fi
...
apply_torch_override() {
    # Tianshu mode: NEVER touch torch versions — the +corex builds are
    # pre-installed and must not be overwritten by CUDA/CPU wheels.
    if [ "$TIANSHU_MODE" -eq 1 ]; then
        echo "[install.sh] Tianshu mode: skipping torch override (preserving +corex builds)."
        return 0
    fi
    ...
    if [ "$PLATFORM_RELAX_TORCHCODEC" -eq 1 ]; then
        ...
        sed -i 's/"torchcodec[>=<~!]+[0-9.]*"/"torchcodec>=0.5"/' "$PYPROJECT_FILE"
    ...
    if [ -n "$TORCH_VERSION" ]; then
        ...
        # Pattern matches both "torch==X.Y.Z" and "torch>=X.Y.Z" (relaxed pins).
...
# In tianshu mode, use pip directly on the system Python instead of uv sync
# (uv sync enforces exact version pins that conflict with +corex builds).
# Usage: tianshu_pip_install [--extra <name>] [--inexact]
tianshu_pip_install() {
    local extra_flag=""
    local extra_name=""
    while [ "$#" -gt 0 ]; do
        case "$1" in
            --extra)
                ...
                ;;
            --inexact)
                # ignored in pip mode — pip doesn't have a sync concept
                shift
                ;;
            *)
                shift
    ...
    if [ -n "$extra_name" ]; then
        echo "[install.sh] Tianshu: uv pip install -e \"${repo_root}[${extra_name}]\""
        uv pip install -e "${repo_root}[${extra_name}]"
    else
        echo "[install.sh] Tianshu: uv pip install -e \"${repo_root}\" --no-deps"
        uv pip install -e "${repo_root}" --no-deps
    fi
}

create_and_sync_venv() {
    # Tianshu mode: install directly into system Python, no venv.
    if [ "$TIANSHU_MODE" -eq 1 ]; then
        install_uv
        echo "[install.sh] Tianshu mode: installing into system Python (no venv)."
        echo "[install.sh] Python: $(python --version 2>&1), path: $(which python)"
        tianshu_pip_install "$@"
        return 0
    fi
...
# Wrapper for uv sync / pip install that routes to the right implementation
# based on TIANSHU_MODE. Usage mirrors uv sync:
#   rlinf_sync [--extra <name>] [--active] [--inexact]
rlinf_sync() {
    local extra_flag=""
    local active_flag=""
    local inexact_flag=""
    ...
    if [ "$TIANSHU_MODE" -eq 1 ]; then
        tianshu_pip_install $extra_flag $inexact_flag
    else
        uv sync $extra_flag $active_flag $inexact_flag $NO_INSTALL_RLINF_CMD
    fi
}
...
install_qwen3_vl_sglang_deps() {
    ...
    rlinf_sync --extra agentic-sglang --inexact --active
...
install_openvla_model() {
    ...
    install_flash_attn
    if [ "$TIANSHU_MODE" -eq 0 ]; then uv pip uninstall pynvml || true; else echo "[install.sh] Tianshu: skipping pynvml uninstall (preserving +corex version)."; fi
}
...
install_openpi_model() {
    case "$ENV_NAME" in
        ...
        franka-franky)
            create_and_sync_venv
            install_common_embodied_deps
            rlinf_sync --extra franka --inexact --active
            if [ "$NO_ROOT" -eq 0 ]; then
    ...
    local py_major_minor site_pkgs
...
EOF
    ...
    if [ "$TIANSHU_MODE" -eq 1 ]; then
        ...
        site_pkgs="$VENV_DIR/lib/python${py_major_minor}/site-packages"
    fi
    cp -r "$site_pkgs/openpi/models_pytorch/transformers_replace/"* \
        "$site_pkgs/transformers/"
...
install_franka_realworld_env() {
    rlinf_sync --extra franka --active
    install_lerobot
...
install_env_only() {
    ...
    case "$ENV_NAME" in
        ...
        franka-franky)
            ...
            ;;
        xsquare_turtle2)
            rlinf_sync --extra xsquare_turtle2 --active
        ...
        embodichain)
            ...
            ;;
        gim_arm)
            rlinf_sync --extra gim_arm --active
            ;;
        dosw1)
            install_dosw1_env
            ;;
...
install_libero_env() {
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        echo "export PYTHONPATH=$(realpath "$libero_dir"):\$PYTHONPATH" >> "$VENV_DIR/bin/activate"
    else
        echo "[install.sh] Tianshu: skipping venv activate write for LIBERO PYTHONPATH."
    fi
}
...
install_d4rl_env() {
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        ...
        echo "[install.sh] Tianshu: skipping venv activate write for MuJoCo LD_LIBRARY_PATH."
    fi
...
install_behavior_env() {
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        ...
        echo "[install.sh] Tianshu: skipping BEHAVIOR-specific torch 2.5.1 install (preserving +corex torch)."
    fi
}
...
install_polaris_env() {
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        ...
        echo "[install.sh] Tianshu: skipping venv activate write for OMNI_KIT_ACCEPT_EULA."
    fi
...
install_franka_env() {
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        ...
        echo "[install.sh] Tianshu: skipping venv activate writes for Franka ROS env vars."
    fi
}
...
install_genesis_env() {
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        ...
        echo "[install.sh] Tianshu: skipping Genesis-specific torch 2.8.0 install (preserving +corex torch)."
    fi
}
...
install_opensora_world_model() {
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        ...
        echo "[install.sh] Tianshu: skipping venv activate write for TensorNVMe LD_LIBRARY_PATH."
    fi
    install_apex
}
...
install_agentic() {
    rlinf_sync --extra agentic-vllm --active
    rlinf_sync --extra agentic-sglang --inexact --active
    if [ "$NO_ROOT" -eq 0 ]; then
        bash $SCRIPT_DIR/sys_deps.sh "$PLATFORM"
    fi
    ...
    if [ "$TIANSHU_MODE" -eq 0 ]; then
        ...
        echo "[install.sh] Tianshu: skipping venv activate write for Megatron PYTHONPATH."
    fi
...
install_docs() {
    rlinf_sync --extra agentic-vllm --active
    rlinf_sync --extra agentic-sglang --inexact --active
    rlinf_sync --extra embodied --active --inexact
...
main() {
    ...
    # When --tianshu is passed, auto-set the platform so configure_platform
    # picks the right tianshu config (skips torch override, flash-attn, etc.).
    if [ "$TIANSHU_MODE" -eq 1 ] && [ "$PLATFORM" != "tianshu" ]; then
        echo "[install.sh] --tianshu detected; switching platform to tianshu."
        PLATFORM="tianshu"
    fi
