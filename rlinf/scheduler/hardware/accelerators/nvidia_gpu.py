...
class NvidiaGPUManager(AcceleratorManager):
    ...
    def get_num_devices():
        ...
        num = NvidiaGPUAcceleratorManager.get_current_node_num_accelerators()
        # 改的：天数(Corex)等非NVIDIA GPU通过IxCuda兼容层暴露CUDA接口，
        # 但NVML无法识别它们。回退到torch.cuda.device_count()获取实际可用设备数。
        if num == 0:
            import torch
    ...
    def get_accelerator_model():
        ...
        try:
            ...
            # 改的：NVML无法识别天数(Corex)等非NVIDIA GPU时，
            # 回退到torch.cuda.get_device_name()获取设备名称。
            import torch
            if torch.cuda.is_available():
    ...
    def get_accelerator_env_var(visible_accelerators: list[str]) -> dict[str, str]:
        ...
        # Simulator env vars
        # Respect an explicit MUJOCO_EGL_DEVICE_ID set by the user/launcher.
        # This is required on machines without per-device EGL (e.g. CoreX/Iluvatar,
        # which only expose a single software EGL device): RLinf would otherwise set
        # MUJOCO_EGL_DEVICE_ID=<gpu_rank> and MuJoCo would fail for every rank > 0.
        if len(visible_accelerators) > 0:
            env_vars["MUJOCO_EGL_DEVICE_ID"] = os.environ.get(
                "MUJOCO_EGL_DEVICE_ID", str(visible_accelerators[0])
            )
