...
class FSDPVlaSftWorker(FSDPSftWorker):
    ...
    @staticmethod
    def _cast_float64_to_float32(x):
        """Cast float64 leaves to float32 on CPU.

        CoreX/Iluvatar silently zeroes float64 tensors on H2D copies, while
        openpi's dataloader yields float64 actions/state. FSDP's
        ``_root_pre_forward`` calls ``_to_kwargs()``, which moves every kwarg
        (including ``data``) to the compute device *before* the model body runs,
        so casting inside ``sft_forward`` is already too late. Cast here instead.
        """
    ...
    def get_train_model_output(self, batch: Any) -> tuple[torch.Tensor, dict[str, Any]]:
        register_pytree_dataclasses(batch)
        batch = tree_map(self._cast_float64_to_float32, batch)
