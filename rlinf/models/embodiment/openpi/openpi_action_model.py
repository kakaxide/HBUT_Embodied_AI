...
class OpenPi0ForRLActionPrediction(PI0Pytorch, BasePolicy):
    ...
    def _no_split_modules(self) -> list[str]:
        if self.config.train_expert_only:
            no_split_modules = [
                ...
                # 改的：NPU 上 FSDP FlatParameter 视图重建时会把 Embedding weight 从 2D 压成 1D，
                # 导致 F.embedding 校验 weight.dim() != 2 失败。单独分出来避免 flatten。
                "Embedding",
            ]
        else:
            no_split_modules = [
    ...
    def sft_forward(self, data, use_action_chunk_loss: bool = False, **kwargs):
        ...
        observation = tree_map(
            lambda x: (
                #改的：只对浮点型转float32（解决天数float64精度警告），布尔型image_masks保持原样，避免jaxtyping类型检查失败
                (
                    torch.as_tensor(x, device=device, dtype=torch.float32).contiguous().clone()
                    if torch.as_tensor(x).is_floating_point()
                    else torch.as_tensor(x, device=device).contiguous().clone()
                )
                if x is not None
                else x
            ),
        ...
        # NOTE: CoreX/Iluvatar GPU silently zeroes float64 tensors on H2D copy
        # (openpi's dataloader yields actions as float64). Cast to float32 on CPU
        # BEFORE moving to device, otherwise the action target becomes all-zero and
        # the flow-matching model collapses to predicting noise (x_0 ~ 0 at inference).
